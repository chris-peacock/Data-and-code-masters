...
for i = 1:numstrain
    ...
    if length(find(strain==strainskip))>0
        ...
        %switch index from 1:3 to 2:4, for diff sample naming
        for j = 2:4
            ...
            %take only even numbered elements (row 2 of 2, modulus in MPa)
            data(1:length(data_pre{1}(2:2:end)),i,j-1) = data_pre{1}(2:2:end)/(10^15);
...
xlim([(min(data_l(1:numstrain))-1) (max(data_l(1:numstrain))+1)])
ylim([ylim_min ylim_max])
path = 'C:\Users\Chris\Desktop\school\Dalhousie\DATA_Chris\AFM_images\height_mod_summary_MASTER\allfigs\mod_figs_fibstrain_2\';
%saveas(figure(2),fullfile(path,['SE',num2str(x+10),'_m_truestrain.jpg']), 'jpeg');
...
dlmwrite([pname,'m_average.txt'],M1);
dlmwrite([pname,'m_std.txt'],M2);