function [dpeak,dpeak_check,RMS] = fourier_profile_general(data,datasize,avrange,flow,fhigh,plot_decision,dcheck)
    ...
    RMS = NaN(S(1),1);
    dpeak = NaN(S(1),1);
    dpeak_check = NaN(S(1),1);
    
    for i = 1:S(1)

                X = squeeze(X_all(i,:))/1000;
                X = X(find(X>=0));

                Y = squeeze(Y_all(i,:));
                Y = Y(find(Y>=0));
                
                Y_av = zeros(1,(length(Y)-avrange));
                ...
                if length(Y)>0
                    ...
                    X1 = X(((avrange/2)+1):(length(X)-(avrange/2)));
                    ...
                    if dcheck
                        ...
                        xlabel('1/\mu m')
                        xlim([0 25])
                        dpeak_check(i) = input('good enough peak finding? (0 for no, 1 for yes):  ');
                    end
                end
    end
end